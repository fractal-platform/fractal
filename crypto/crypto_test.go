...
func TestBn256PubKey(t *testing.T) {
	...
	Equal(t, pkBytes, pk.Marshal())
}

func TestBn256PriKey(t *testing.T) {
	_, sk, _ := NewKeys(BLS)
	skBytes := sk.Marshal()
	t.Logf("byte len is %d", len(skBytes))
	NotNil(t, skBytes)

	err := sk.Unmarshal(skBytes)
	Nil(t, err)

	priKey := new(bn256PriKey)
	priKey.Unmarshal(skBytes)
	Equal(t, priKey, sk)
}

func TestBn256PriKey_Sign(t *testing.T) {
	pk, sk, err := NewKeys(BLS)
	Nil(t, err)
	...
	sig, _ := sk.Sign(msg)
	Equal(t, pk.Verify(msg, sig), true)

	rand.Read(msg)
	Equal(t, pk.Verify(msg, sig), false)
}
