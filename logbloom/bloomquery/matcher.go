...
func calcBloomIndexes(b []byte) bloomIndexes {
	...
	for i := 0; i < len(idxs); i++ {
		idxs[i] = (uint(b[2*i])<<8)&(types.BloomBitLength-1) + uint(b[2*i+1])
	}
	return idxs
}
...
func NewMatcher(filters [][][]byte) *Matcher {
	...
	m := &Matcher{
		sectionSize: params.BloomBitsSize,
...
func (m *Matcher) Start(ctx context.Context, db dbwrapper.Database, sectionIds []uint64, begin uint64, end uint64, results chan uint64) (*MatcherSession, error) {
	...
	sink := m.run(db, sectionIds, cap(results), session)
...
func (m *Matcher) run(db dbwrapper.Database, sectionIds []uint64, buffer int, session *MatcherSession) chan *partialMatches {
	...
	go func() {
		...
		for _, sectionIds := range sectionIds {
			select {
			case <-session.quit:
				return
			case source <- &partialMatches{sectionIds, bytes.Repeat([]byte{0xff}, int(params.BloomByteSize))}:
			}
		}
	}()
			...
			case subres, ok := <-process:
				...
				for _, bloomSinks := range sectionSinks {
					...
					for _, bitSink := range bloomSinks {
						...
						if andVector == nil {
							andVector = make([]byte, len(subres.bitset))
							copy(andVector, data)
						} else {
				...
				if orVector == nil {
					orVector = make([]byte, len(subres.bitset))
				}
...
func (s *MatcherSession) getQueryTaskLoop(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
...
func (s *MatcherSession) MultiGetQueryTaskLoop(ctx context.Context, bloomRequests chan chan *Retrieval) {
	for i := 0; i < bloomFilterThreads; i++ {
		go s.getQueryTaskLoop(bloomRetrievalBatch, bloomRetrievalWait, bloomRequests)
	}
}
