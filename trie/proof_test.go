...
// test all variations.
func makeProvers(trie *Trie) []func(key []byte) *dbwrapper.MemDatabase {
	var provers []func(key []byte) *dbwrapper.MemDatabase

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *dbwrapper.MemDatabase {
		proof := dbwrapper.NewMemDatabase()
...
func BenchmarkProve(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		proofs := dbwrapper.NewMemDatabase()
...
func BenchmarkVerifyProof(b *testing.B) {
	...
	root := trie.Hash()
	var keys []string
	var proofs []*dbwrapper.MemDatabase
	for k := range vals {
		keys = append(keys, k)
		proof := dbwrapper.NewMemDatabase()
