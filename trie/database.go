...
type Database struct {
	diskdb dbwrapper.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb dbwrapper.Database) *Database {
	return &Database{
		diskdb:    diskdb,
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			...
			if batch.ValueSize() > dbwrapper.IdealBatchSize {
...
func (db *Database) commit(hash common.Hash, batch dbwrapper.Batch) error {
