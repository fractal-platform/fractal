...
//	conn, err := net.DialTimeout("tcp", srv.RwListenAddr, 5*time.Second)
//	if err != nil {
...
//	node := &discover.Node{ID: remid, IP: tcpAddr.IP, DiscPort: uint16(tcpAddr.Port), RwPort: uint16(tcpAddr.Port)}
//	srv.AddPeer(node)
//
//	select {
//	case conn := <-accepted:
