...
func newTable(t transport, ourID NodeID, ourAddr *net.UDPAddr, nodeDBPath string, bootnodes []*Node, logger log.Logger) (*Table, error) {
	...
	tab := &Table{
		net:        t,
		db:         db,
		blacklist:  make(map[NodeID]*Node),
...
func (tab *Table) AddBlack(target *Node) {
	tab.mutex.Lock()
	defer tab.mutex.Unlock()

	tab.blacklist[target.ID] = target
	tab.db.addBlack(target)

	// remove it from bucket
	tab.deleteInBucket(tab.bucket(target.sha), target)
}

func (tab *Table) RemoveBlack(target *Node) {
	tab.mutex.Lock()
	defer tab.mutex.Unlock()

	delete(tab.blacklist, target.ID)
	tab.db.deleteBlack(target)
}
...
func (tab *Table) loadBlacklist() {
	for _, node := range tab.db.loadBlacklist() {
		tab.blacklist[node.ID] = node
	}
}
...
func (tab *Table) add(n *Node) {
	...
	if _, ok := tab.blacklist[n.ID]; ok {
		// no add for node in blacklist
		tab.logger.Info("reject to add node in blacklist", "node", n)
		return
	}
