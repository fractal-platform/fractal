...
//
func (db *nodeDB) loadBlacklist() []*Node {
	blacklist := []*Node{}
	it := db.lvl.NewIterator(util.BytesPrefix(nodeDBBlackPrefix), nil)
	for it.Next() {
		node := new(Node)
		if err := rlp.DecodeBytes(it.Value(), node); err != nil {
			log.Error("Failed to decode node RLP", "err", err)
			return nil
		}
		node.sha = crypto.Keccak256Hash(node.ID[:])
		blacklist = append(blacklist, node)
	}
	return blacklist
}

//
func (db *nodeDB) addBlack(node *Node) error {
	blob, err := rlp.EncodeToBytes(node)
	if err != nil {
		return err
	}
	return db.lvl.Put(append(nodeDBBlackPrefix, node.ID[:]...), blob, nil)
}

//
func (db *nodeDB) deleteBlack(node *Node) {
	db.lvl.Delete(append(nodeDBBlackPrefix, node.ID[:]...), nil)
}
