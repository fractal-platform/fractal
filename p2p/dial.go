...
func (t TCPDialer) Dial(dest *discover.Node) (net.Conn, error) {
	addr := &net.TCPAddr{IP: dest.IP, Port: int(dest.RwPort)}
...
func (t KCPDialer) Dial(dest *discover.Node) (net.Conn, error) {
	addr := &net.UDPAddr{IP: dest.IP, Port: int(dest.RwPort)}
...
type discoverTable interface {
	...
	AddBlack(target *discover.Node)
	RemoveBlack(target *discover.Node)
}
...
func (s *dialstate) newTasks(nRunning int, peers map[discover.NodeID]*Peer, now time.Time) []task {
	...
	addDial := func(flag connFlag, n *discover.Node) bool {
		if err := s.checkDial(n, peers); err != nil {
			log.Debug("Skipping dial candidate", "id", n.ID, "addr", &net.TCPAddr{IP: n.IP, Port: int(n.RwPort)}, "err", err)
			return false
		}
		s.dialing[n.ID] = flag
	...
	for id, t := range s.static {
		...
		case errNotWhitelisted, errSelf:
			log.Warn("Removing static dial candidate", "id", t.dest.ID, "addr", &net.TCPAddr{IP: t.dest.IP, Port: int(t.dest.RwPort)}, "err", err)
...
func (t *dialTask) resolve(srv *Server) bool {
	...
	t.resolveDelay = initialResolveDelay
	t.dest = resolved
	log.Debug("Resolved node", "id", t.dest.ID, "addr", &net.TCPAddr{IP: t.dest.IP, Port: int(t.dest.RwPort)})
	return true
}
...
func (t *dialTask) String() string {
	return fmt.Sprintf("%v %x %v:%d", t.flags, t.dest.ID[:8], t.dest.IP, t.dest.RwPort)
}
