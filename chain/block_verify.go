...
func (bc *BlockChain) VerifyBlock(block *types.Block, checkGreedy bool) (types.Blocks, common.Hash, int, common.Hash, error) {
	...
	if block.FullHash() == bc.genesisBlock.FullHash() {
		return nil, common.Hash{}, 0, common.Hash{}, nil
	}

	// check depend block exists
	dependBlockHash, err := bc.VerifyBlockDepend(block)
	if err != nil {
		depth := bc.addFutureBlock(dependBlockHash, block)
		bc.logger.Error("Block verify failed", "hash", block.FullHash(), "miss", dependBlockHash, "depth", depth, "err", err)
		return nil, dependBlockHash, depth, common.Hash{}, err
	}
	...
	if parentBlock.SimpleHash() != block.Header.ParentHash {
		...
		return nil, common.Hash{}, 0, common.Hash{}, ErrNotConfirmParentBlock
	}
	...
	//if block.Header.Height <= currentBlock.Header.Height && checkGreedy == true {
	...
	//		return nil, common.Hash{}, 0, common.Hash{}, ErrBlockNotMeetGreedy
	//	}
	//}
	...
	if block.Header.GasLimit > params.MaxGasLimit {
		...
		return nil, common.Hash{}, 0, common.Hash{}, ErrInvalidGasLimit
	}
	// Verify that the gasUsed is <= gasLimit
	if block.Header.GasUsed > block.Header.GasLimit {
		bc.logger.Error("Block verify failed", "err", ErrInvalidGasUsed)
		return nil, common.Hash{}, 0, common.Hash{}, ErrInvalidGasUsed
	}
	...
	if block.Header.Height != parentBlock.Header.Height+1 {
		...
		return nil, common.Hash{}, 0, common.Hash{}, ErrBlockHeightError
	}
	if block.Header.Round <= parentBlock.Header.Round {
		bc.logger.Error("Block verify failed", "err", ErrBlockRoundTooLow)
		return nil, common.Hash{}, 0, common.Hash{}, ErrBlockRoundTooLow
	}
	...
	if expected.Cmp(block.Header.Difficulty) != 0 {
		...
		return nil, common.Hash{}, 0, common.Hash{}, ErrBlockConsensusError
	}
	...
	if !bc.chainConfig.BlockSigFake {
		...
		if !key.Verify(block.SignHashByte(), block.Header.Sig) {
			return nil, common.Hash{}, 0, common.Hash{}, ErrBlockSigError
		}

		// * Verify FullSig[]
		if !key.Verify(block.FullHash().Bytes(), block.Header.FullSig) {
			return nil, common.Hash{}, 0, common.Hash{}, ErrBlockFullSigError
		}
	}
	...
	// Check if Compliance with check point rule
	if checkGreedy && !bc.meetCheckPointRule(block) {
		return nil, common.Hash{}, 0, common.Hash{}, ErrBlockNotMeetCheckPoint
	}

	for _, tx := range block.Body.Transactions {
		_, err := types.Sender(bc.txSigner, tx)
		if err != nil {
			return nil, common.Hash{}, 0, common.Hash{}, ErrPackTxSignError
		}
	}
	...
	if block.Header.TxHash != types.DeriveSha(types.Transactions(block.Body.Transactions)) {
		...
		return nil, common.Hash{}, 0, common.Hash{}, ErrBlockTxHashError
	}
	...
	for i := range txPackageHashes {
		...
		if pkg == nil {
			...
			return nil, common.Hash{}, 0, txPackageHashes[i], ErrBlockTxPackageMissing
		}
	...
	return confirmBlocks, common.Hash{}, 0, common.Hash{}, nil
}
